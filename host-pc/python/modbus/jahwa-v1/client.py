...
async def send_payload(client, endian):
    ...
    ##                   Define Data Builder                   ##         
    #############################################################
    # Get word and byte order
    word_endian, byte_endian = endian

    ######################### Write I2C #########################
    # Create payload builder
    write_i2c_builder = BinaryPayloadBuilder(
        wordorder=word_endian,
        byteorder=byte_endian,
    )
    
    # Define address and data length
    addr_len = 10
    data_len = 10

    # Populate payload with data
    # Add the slave address
    write_i2c_builder.add_32bit_uint(0x0)
    ...
    for i in range(addr_len):
        write_i2c_builder.add_8bit_uint(0x0)
    # Add the data buffer
    ...
    ########################## Read I2C #########################
    # Create payload builder
    read_i2c_builder = BinaryPayloadBuilder(
        wordorder=word_endian,
        byteorder=byte_endian,
    )
    ...
    # Populate payload with data
    # Add the slave address
    read_i2c_builder.add_32bit_uint(0x0)
    # Add the address length
    read_i2c_builder.add_32bit_uint(0x0)
    # Add the data length
    read_i2c_builder.add_32bit_uint(0x0)
    # Add the address buffer
    for i in range(addr_len):
        read_i2c_builder.add_8bit_uint(0x0)
    # Add the data buffer
    for i in range(data_len):
        read_i2c_builder.add_8bit_uint(0x0)
    ...
    ####################### Pattern Load ########################
    # Create payload builder
    pattern_load_builder = BinaryPayloadBuilder(
        wordorder=word_endian,
        byteorder=byte_endian,
    )

    # Define builder parameters
    wr_packet_size = 10
    rd_packet_size = 10
    block_count = 10
    measure_count = 10
    ...
    # Populate payload with data
    # Add command ID
    pattern_load_builder.add_32bit_uint(0x0)
    # Add write slave address
    pattern_load_builder.add_32bit_uint(0x0)
    # Add read slave address
    pattern_load_builder.add_32bit_uint(0x0)
    # Add write packet size
    pattern_load_builder.add_32bit_uint(0x0)
    # Add read packet size
    pattern_load_builder.add_32bit_uint(0x0)
    # Add block count
    pattern_load_builder.add_32bit_uint(0x0)
    # Add measure count
    ...
    pattern_load_builder.add_32bit_uint(0x0)
    # Add delay from write to measure interval
    pattern_load_builder.add_32bit_uint(0x0)
    # Add write data buffer
    for i in range(wr_buffer_size):
        pattern_load_builder.add_8bit_uint(0x0)

    ######################## Pattern Run ########################
    # Create payload builder
    pattern_run_builder = BinaryPayloadBuilder(
        wordorder=word_endian,
        byteorder=byte_endian,
    )
    ...
    # Populate payload with data
    # Add command ID
    pattern_run_builder.add_32bit_uint(0x0)

    ##################### Pattern Get Data ######################
    # Create payload builder
    pattern_get_data_send_builder = BinaryPayloadBuilder(
        wordorder=word_endian,
        byteorder=byte_endian,
    )
    ...
    pattern_get_data_send_builder.add_32bit_uint(0x0)
    # Add LDO Channel 0 current buffer size
    pattern_get_data_send_builder.add_32bit_uint(0x0)
    # Add LDO Channel 0 voltage buffer size
    pattern_get_data_send_builder.add_32bit_uint(0x0)
    # Add LDO Channel 1 current buffer size
    pattern_get_data_send_builder.add_32bit_uint(0x0)
    # Add LDO Channel 1 voltage buffer size
    pattern_get_data_send_builder.add_32bit_uint(0x0)

    ##################### Measure Position ######################
    # Create payload builder
    measure_position_builder = BinaryPayloadBuilder(
        wordorder=word_endian,
        byteorder=byte_endian,
    )
    ...
    # Populate payload with data
    # Add command ID
    measure_position_builder.add_32bit_uint(0x0)

    ###################### Set LDO Voltage ######################
    # Create payload builder
    set_ldo_voltage_builder = BinaryPayloadBuilder(
        wordorder=word_endian,
        byteorder=byte_endian,
    )
    ...
    # Populate payload with data
    # Add LDO channel
    set_ldo_voltage_builder.add_32bit_uint(0x0)

    # Add LDO voltage
    set_ldo_voltage_builder.add_64bit_float(0x0)
