...
def encode_packet(type, *args, **kwargs):
    ...
    ##############################################################################################
        ...
        if (sequence == 'gpio_write_sdn1') or (sequence == 'gpio_write_sdn2') or \
            ...
            (sequence == 'gpio_write_relay2') or (sequence == 'spi_config_dac') or \
        ...
        # I2C Meissner chip ID
            ...
            # Get data
            chip_id = kwargs.get('chip_id') if kwargs.get('chip_id') is not None \
                else 0
            # Create packet
            packet = struct.pack(
                '2s 2s 2s c I I',
...
def decode_packet(packet, **kwargs):
    ...
    ##############################################################################################
        ...
        if (sequence == measurement_sequence_dict['gpio_write_sdn1']) or \
            ...
            (sequence == measurement_sequence_dict['gpio_write_sdn3']) or \
            (sequence == measurement_sequence_dict['gpio_write_relay1']) or \
            (sequence == measurement_sequence_dict['gpio_write_relay2']):
        ...
        # I2C Meissner Chip ID
        elif sequence == measurement_sequence_dict['i2c_meissner_chip_id']:
        ...
        # Laser trigger
        elif sequence == measurement_sequence_dict['laser_trigger']:
            transaction_id, protocol_id, length, unit_id, sequence, div_ratio, actv_count = \
            struct.unpack(
                '2s 2s 2s c I I I',
                packet
            )
