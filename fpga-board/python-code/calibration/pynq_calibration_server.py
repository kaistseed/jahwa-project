    ...
    while True:
        ...
        # FIXME: Need to be updated
            ...
            if packet['sequence'] == 1:
                print("Received GPIO write SDN1 packet from client")
                # Get I/O state
                io_state = packet['io_state']
                
                # Run toggle LED sequence
                microblaze.gpio_write_sdn1(io_state)
            ...
            # SDN2 I/O
                ...
                print("Received GPIO write SDN2 packet from client")
                # Get I/O state
                io_state = packet['io_state']
                
                # Run toggle LED sequence
                microblaze.gpio_write_sdn2(io_state)
            ...
            # SDN3 I/O
                ...
                print("Received GPIO write SDN3 packet from client")
                # Get I/O state
                io_state = packet['io_state']
                
                # Run toggle LED sequence
                microblaze.gpio_write_sdn3(io_state)
            ...
            # DAC Configutaion
                ...
                # Get DAC channel and channel code
                channel = packet['channel']
                code = packet['code']

                # Run configure DAC sequence
                microblaze.spi_config_dac(channel, code)
            ...
            # ADC Read
                ...
                print("Received read ADC sequence packet from client")
                print()
                # Get number of samples and sample interval
                num_of_samples = packet['num_of_samples']
                ...
                # Compute ADC voltage
                adc_voltage = [((value/65536.0)*4.096) for value in adc_value]
            ...
            # I2C Meissner chip ID
                ...
                print("Received read Meissner chip ID packet from client")
                ...
                # Print packet
                print("Chip ID packet: {}".format(packet))
                print()
            ...
            # Relay 1 I/O
                ...
                print("Received GPIO write relay 1 packet from client")
                # Get I/O state
                io_state = packet['io_state']

                # Run toggle LED sequence
                microblaze.gpio_write_relay1(io_state)
            ...
            # Relay 2 I/O
                ...
                print("Received GPIO write relay 2 packet from client")
                # Get I/O state
                io_state = packet['io_state']

                # Run toggle LED sequence
                microblaze.gpio_write_relay2(io_state)
            ...
            # Laser trigger
                ...
                print("Received laser trigger packet from client")
                # Get division ratio and active count
                div_ratio = packet['div_ratio']
                actv_count = packet['actv_count']

                # Run configure DAC sequence
                microblaze.laser_trigger(div_ratio, actv_count)
...
#                                        Define TCP Server                                       #
##################################################################################################
async def tcp_server(server_addr, server_port, microblaze):
...
if __name__ == "__main__":
    ...
    ol = Overlay("./bitstream/jahwa_daq_system_v3.bit")
